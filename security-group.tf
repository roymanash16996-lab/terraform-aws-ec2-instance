module "security_group" {
  ...
  vpc_id        = local.vpc_id
  name          = lookup(each.value, "security_group_name", local.resource_name)
  description   = lookup(each.value, "security_group_description", "Security group for instance ${local.resource_name} for ${each.key}")
